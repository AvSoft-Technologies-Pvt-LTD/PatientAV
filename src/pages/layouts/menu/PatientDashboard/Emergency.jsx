...
const Emergency = () => {
  ...
  useEffect(() => { axios.get('https://mocki.io/v1/70ebb83b-4fde-42d5-9e5a-d3372b468bcf').then(r => setData(r.data)).catch(() => toast.error('Failed to fetch data')); }, []);
  ...
  const handleSubmit = async () => { try { await axios.post('https://mocki.io/v1/70ebb83b-4fde-42d5-9e5a-d3372b468bcf', { ambulanceType: type, category: cat, equipment: equip, pickupLocation: pickup, dropLocation: '', date: format(date, 'yyyy-MM-dd') }); toast('Connecting....', { className: 'Toastify__toast--connecting' }); } catch { toast('Connecting....', { className: 'Toastify__toast--connecting' }); } };
