...
const formTypes = {
  all: { id: 'all', name: 'All', icon: Menu, color: 'from-[var(--primary-color)] to-[var(--accent-color)]', description: 'Show all forms together' },
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4">
          ...
          {showPatientDetails && (
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-100 mb-4 animate-fadeIn relative">
              <button onClick={() => setShowPatientDetails(false)} className="absolute top-2 right-2 text-gray-400 hover:text-gray-600 transition-colors"><X className="w-4 h-4" /></button>
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div className="flex items-center gap-4 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h2 className="h4-heading truncate">{getPatientName()}</h2>
                    <div className="flex items-center gap-2 text-[var(--accent-color)] text-sm truncate">
                      <Mail className="w-3 h-3" />
                      <span className="truncate">{patient?.email || 'N/A'}</span>
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-2 flex-shrink-0 w-full md:w-auto">
                  <div className="flex items-center gap-2 text-[var(--primary-color)] bg-white/60 rounded px-2 py-1">
                    <Phone className="w-3 h-3 text-[var(--accent-color)]" />
                  ...
                  <div className="flex items-center gap-2 text-[var(--primary-color)] bg-white/60 rounded px-2 py-1">
                    <FileText className="w-3 h-3 text-[var(--accent-color)]" />
          ...
          {!showPatientDetails && (
            <div className="mb-4">
              <button onClick={() => setShowPatientDetails(true)} className="flex items-center gap-2 px-4 py-2 bg-[var(--primary-color)] text-white rounded-lg hover:bg-opacity-90 transition-colors get-details-animate">
                <ChevronLeft className="w-4 h-4" />Show Patient Details
              </button>
            </div>
          )}
          <div className="flex items-center justify-between">
            <div className="hidden md:flex items-center gap-2">
              {Object.values(formTypes).map(formType => {
                const Icon = formType.icon, isActive = activeForm === formType.id;
                return (
                  <button key={formType.id} onClick={() => setActiveForm(formType.id)}
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all duration-200 ${isActive ? 'bg-[var(--primary-color)] text-white shadow-md' : 'bg-white text-[var(--primary-color)] hover:bg-gray-50 border border-gray-200'}`}>
                    <Icon className="w-4 h-4" /><span className="text-sm">{formType.name}</span>
                  </button>
                );
              })}
            </div>
            <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden flex items-center gap-2 px-4 py-2 bg-[var(--primary-color)] text-white rounded-lg">
              <Menu className="w-4 h-4" /><span className="text-sm">Forms</span>
            </button>
            <button className="btn btn-primary text-sm get-details-animate" onClick={printAllForms}>
            ...
            <div className="md:hidden mt-4 bg-white rounded-lg shadow-lg border border-gray-200 p-4 animate-slideIn">
              <div className="grid grid-cols-2 gap-2">
                {Object.values(formTypes).map(formType => {
                  const Icon = formType.icon, isActive = activeForm === formType.id;
                  return (
                    <button key={formType.id} onClick={() => { setActiveForm(formType.id); setIsMobileMenuOpen(false); }}
                      className={`flex items-center gap-2 px-3 py-2 rounded-lg font-medium transition-all duration-200 text-sm ${isActive ? 'bg-[var(--primary-color)] text-white shadow-md' : 'bg-gray-50 text-[var(--primary-color)] hover:bg-gray-100'}`}>
                      <Icon className="w-4 h-4" /><span>{formType.name}</span>
                    </button>
                  );
                })}
              </div>
